...
describe("plugin-sdk subpath exports", () => {
  ...
  it("keeps removed bundled-channel prefixes out of the public sdk list", () => {
    ...
    // This narrow Telegram helper remains public because shared config loading
    // still consumes it through the plugin-sdk boundary.
    const allowedExceptions = new Set(["telegram-command-config"]);
    expect(banned.filter((subpath) => !allowedExceptions.has(subpath))).toEqual([]);
  });

...
import { registerDockerSetupBrowserExecTests } from "./docker-setup.browser-exec.e2e.test-support.js";
import {
...
describe("scripts/docker/setup.sh", () => {
  ...
  registerDockerSetupBrowserExecTests(() => requireSandbox(sandbox), runDockerSetup);
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain('if [ -L "$path" ]');
  ...
  it("bootstraps node-owned runtime dirs before starting the gateway", async () => {
    ...
    expect(compose).toContain('if [ -L "$$path" ]');
    expect(compose).toContain("/home/node/.local/share/pnpm");
    expect(compose).toContain("/home/node/.npm-global/bin");
    expect(compose).toContain("/home/node/go/bin");
    expect(compose).toContain(
      '/usr/bin/find -P "$$root" -xdev -execdir /usr/bin/chown -h node:node {} +',
    );
